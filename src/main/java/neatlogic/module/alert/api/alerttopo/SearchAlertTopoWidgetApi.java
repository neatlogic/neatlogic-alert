...
public class SearchAlertTopoWidgetApi extends PrivateApiComponentBase {
    ...
    public String getToken() {
        return "/alert/topo/widget/search";
    }
    ...
    @Output({@Param(explode = BasePageVo.class),
            @Param(name = "tbodyList", explode = AlertTopoWidgetVo.class)})
    @Description(desc = "搜索拓扑图元")
    @Override
    public Object myDoService(JSONObject jsonObj) {
        AlertTopoWidgetVo widgetVo = JSON.toJavaObject(jsonObj, AlertTopoWidgetVo.class);
        List<AlertTopoWidgetVo> widgetList = null;
        int rowNum = alertTopoWidgetMapper.searchWidgetCount(widgetVo);
        if (rowNum > 0) {
            widgetVo.setRowNum(rowNum);
            widgetList = alertTopoWidgetMapper.searchWidget(widgetVo);
        }

        return TableResultUtil.getResult(widgetList, widgetVo);
    }
}
