...
@Component
public class AlertUnActionEventHandler extends AlertEventHandlerBase {
    @Resource
    ...
    public String getLabel() {
        return $.t("term.alert.event.unactionhandlername");
    }

    @Override
    public String getIcon() {
        return "tsfont-forbid";
    }

    @Override
    public String getDescription() {
        return $.t("term.alert.event.unactionhandlerdesc");
    }
    ...
    protected AlertVo myTrigger(AlertEventHandlerVo alertEventHandlerVo, AlertEventPluginVo alertEventPluginVo, AlertVo alertVo, AlertEventHandlerAuditVo alertEventHandlerAuditVo, AlertEventStatusVo alertEventStatusVo) throws AlertEventHandlerTriggerException {
        ...
        if (CollectionUtils.isNotEmpty(actionList)) {
            for (int i = 0; i < actionList.size(); i++) {
                ...
                alertActionMapper.deleteAlertActionRel(alertVo.getId(), action);
            }
        }
        return alertVo;
    }
