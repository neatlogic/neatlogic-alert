...
@Component
public class AlertMarkEventHandler extends AlertEventHandlerBase {
    @Resource
        ...
        if (CollectionUtils.isNotEmpty(markList)) {
            ...
            for (Object mark : markList) {
                if (mark instanceof String) {
                    ...
                    markVo.setName(mark.toString());
                    alertMarkMapper.insertAlertMark(markVo);
                    alertMarkMapper.insertAlertAlertMark(alertVo.getId(), markVo.getUuid());
                } else if (mark instanceof JSONObject) {
                    AlertMarkVo markVo = JSON.parseObject(mark.toString(), AlertMarkVo.class);
                    alertMarkMapper.insertAlertAlertMark(alertVo.getId(), markVo.getUuid());
                }
            }
